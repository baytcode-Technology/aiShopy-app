...
} from '@/components/store/ProductListRow'
...
type StatusFilter = 'all' | ProductListStatus
...
export default function ProductsScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((p) => {
      ...
      const matchesStatus =
        statusFilter === 'all' || resolveProductStatus(p) === statusFilter
    ...
    <View className="pb-2">
      ...
      <View className="flex-row items-center gap-1 mb-1 px-1">
        {STATUS_TABS.map((tab) => {
          ...
          return (
            <Pressable key={tab.key} onPress={() => setStatusFilter(tab.key)} className="mr-1">
              <View
                className={`px-3.5 py-2 rounded-lg ${active ? 'bg-gray-200' : 'bg-transparent'}`}
              >
                <Text
