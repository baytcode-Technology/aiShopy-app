...
export function CategoryActiveBadge({ isActive }: Props) {
  const status: ProductStatus = isActive ? 'active' : 'unlisted'
  const theme = PRODUCT_STATUS_THEME[status]
  return (
    <View className="px-2.5 py-1 rounded-full" style={{ backgroundColor: theme.badgeBg }}>
